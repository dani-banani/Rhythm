playsound minecraft:entity.camel.dash master @a ~ ~ ~ 0.05 2
execute if score $now dani.rhythm.pulse matches 0..40 run scoreboard players set @s dani.rhythm.pulse.stage 1
execute if score $now dani.rhythm.pulse matches 41..60 run scoreboard players set @s dani.rhythm.pulse.stage 2
execute if score $now dani.rhythm.pulse matches 61..100 run scoreboard players set @s dani.rhythm.pulse.stage 3
execute if score $now dani.rhythm.pulse matches 101..140 run scoreboard players set @s dani.rhythm.pulse.stage 4
execute if score $now dani.rhythm.pulse matches 141..160 run scoreboard players set @s dani.rhythm.pulse.stage 5
execute if score $now dani.rhythm.pulse matches 161..200 run scoreboard players set @s dani.rhythm.pulse.stage 6
execute if score $now dani.rhythm.pulse matches ..40 run scoreboard players set @s dani.rhythm.pulse.deathtimer 200
execute if score $now dani.rhythm.pulse matches 161.. run scoreboard players set @s dani.rhythm.pulse.deathtimer 200
execute if score $now dani.rhythm.pulse matches 41..160 if score @s dani.rhythm.pulse.deathtimer matches 0.. run scoreboard players set @s dani.rhythm.pulse.deathtimer -1
scoreboard players operation $changeInStage dani.Global = @s dani.rhythm.pulse.stage
scoreboard players operation $changeInStage dani.Global -= @s dani.rhythm.pulse.stage.old.tick
scoreboard players operation @s dani.rhythm.pulse.stage.old.tick = @s dani.rhythm.pulse.stage
data remove storage dani:player movement_speed
execute store result score $speed dani.Global run attribute @s minecraft:movement_speed base get 1000
execute if score $changeInStage dani.Global matches 1.. run function dani:rhythm/systems/pulse/stage/increase
execute if score $changeInStage dani.Global matches ..-1 run function dani:rhythm/systems/pulse/stage/decrease
execute if score @s dani.rhythm.pulse.stage matches 3 run data modify storage dani:player movement_speed set value 0.1d
function dani:pack/player/attribute/set_speed with storage dani:player
